...
func (s *Storage) GetList(ctx context.Context, key string, opts storage.ListOptions, listObj runtime.Object) error {
	...
	for _, r := range results {
		if r.shouldAppend {
			...
			// Untyped lists use an interface element type (e.g. []resource.Object); those
			// objects implement the interface via pointer receivers, so the pointer must be
			// appended directly — dereferencing would yield a value that does not satisfy
			// the interface and panic in reflect.Append.
